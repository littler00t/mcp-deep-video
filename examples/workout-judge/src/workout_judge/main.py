...
from .agent import VideoAnalysisDeps, workout_agent
...
async def _run_analysis(video_path: Path) -> None:
    console.print(f"\n[bold cyan]Workout Judge[/bold cyan] \u2014 analyzing [yellow]{video_path.name}[/yellow]\n")
    ...
    console.print(
        f"[dim]Video: {meta['width']}x{meta['height']}  "
        f"{meta['fps']:.1f} fps  "
        f"{meta['duration_seconds']:.1f}s  "
        f"({meta['total_frames']} frames)[/dim]\n"
    )

    comp = GridCompositor()
    debug_dir = video_path.parent / ".mcp_debug"
    dbg = DebugWriter(debug_dir, global_debug=True)
    ...
    prompt = (
        "Please analyze the workout technique in this video. "
        "Follow the required analysis sequence: overview first, then sections, then precise frames."
    )

    tool_history: list[str] = []
    current_spinner_text: list[str] = ["Initializing..."]
    ...
    with Live(console=console, refresh_per_second=8) as live:
        ...
        async with workout_agent.iter(prompt, deps=deps) as agent_run:
            async for node in agent_run:
                if workout_agent.is_call_tools_node(node):
